...
function confirmAndDelete(userId) {
  if (confirm("Êtes-vous sûr de vouloir supprimer cet utilisateur ?")) {
    ...
    fetch("?controller=set&action=retirerPersonne&id=" + personId, {
      method: "POST",
      headers: {
      ...
      .then((data) => {
        console.log(data);
        // Redirection vers l'action list personne après la suppression
        window.location.href =
          "?controller=authentification&action=listePersonne";
      })
      .catch((error) => {
