$(function () {
  ...
  // data for demo only
  var initialDataPageViewsPerSecond = [
    1, 2, 4, 4, 3, 2, 1, 5, 1, 2, 2, 0, 0, 1, 2, 4, 5, 3, 4, 2, 2, 2, 2, 0, 1,
    2, 4, 4, 4, 5, 5, 1, 2, 4, 1, 1, 1, 0, 0, 1, 2, 3, 3, 5, 5, 2, 0, 1, 1, 0,
    2, 2, 2, 0, 4, 1, 4, 4, 2, 2,
  ];
  var initialDataPageViewsPerMinute = [
    110, 107, 122, 107, 128, 108, 100, 110, 118, 93, 95, 112, 108, 95, 96, 114,
    107, 105, 124, 104, 131, 94, 109, 110, 108, 99, 104, 90, 104, 109, 89, 121,
    118, 93, 109, 113, 106, 100, 101, 119, 76, 137, 112, 104, 98, 89, 104, 96,
    120, 111, 108, 95, 93, 100, 101, 110, 98, 105, 107, 135,
  ];
  ...
  var usersDeviceDoughnutChart = new CanvasJS.Chart(
    "users-device-doughnut-chart",
    {
      ...
      title: {
        ...
        verticalAlign: "center",
        text: "graphData[0].total", // Accédez au total depuis le premier élément du tableau
      },
      toolTip: {
        cornerRadius: 0,
        fontStyle: "normal",
        contentFormatter: function (e) {
          return (
            ...
            ": " +
            Math.round(e.entries[0].dataPoint.y) +
            "% (" +
  ...
  var totalServices =
    serviceStatistics.statutaire + serviceStatistics.complementaire;
    ...
    data: [
      {
        ...
        type: "pie",
        dataPoints: [
          { y: serviceStatistics.statutaire, name: "STAT" },
          { y: serviceStatistics.complementaire, name: "COMP" },
        ],
      },
    ],
  });
    ...
    legend: {
      ...
      itemTextFormatter: function (e) {
        return (
          e.dataPoint.name +
          ": " +
          Math.round((e.dataPoint.y / activeUsers) * 100) +
          "%"
        );
      },
    },
    toolTip: {
      ...
      contentFormatter: function (e) {
        return (
          e.entries[0].dataPoint.name +
          ": " +
          Math.round((e.entries[0].dataPoint.y / activeUsers) * 100) +
          "% (" +
          e.entries[0].dataPoint.y +
          ")"
        );
  ...
  //     usersDeviceDoughnutChart.options.data[0].dataPoints[i].y =
  //       data[dataIndex].device[i].users;
  ...
  function updateUsersMediumPieChart(dataIndex) {
    for (var i = 0; i < data[dataIndex].trafficMedium.length; i++)
      usersMediumPieChart.options.data[0].dataPoints[i].y =
        data[dataIndex].trafficMedium[i].users;
  ...
  function updateUsersCategoryChart(dataIndex) {
    for (var i = 0; i < data[dataIndex].categories.length; i++)
      usersCategoryPieChart.options.data[0].dataPoints[i].y =
        data[dataIndex].categories[i].users;
  ...
  function updateChartsAtRandomIntervals() {
    ...
    updateChartsInterval = generateRandomNumber(
      updateChartsIntervalLowerLimit,
      updateChartsIntervalUpperLimit
    );

    updateCharts(dataIndex);

    if (timeoutIdUpdateCharts) clearTimeout(timeoutIdUpdateCharts);
  ...
  function populatePageViewsCharts() {
    ...
    for (var i = 0; i < 60; i++) {
      ...
      pageViewsPerSecondDataPoints.push({
        x: time1,
        y: initialDataPageViewsPerSecond[i],
      });
      pageViewsPerMinuteDataPoints.push({
        x: time2,
        y: initialDataPageViewsPerMinute[i],
      });
    }
  }
  ...
  function updatePageViewsCharts() {
    ...
    yValuePageViewsPerSecond = generateRandomNumber(
      pageViewsPerSecondLowerLimit,
      pageViewsPerSecondUpperLimit
    );
